function [ dist_sort_oneshot,data_output ] = sort_data_3d( data_input,Nt,trace_num,trace_num_pershot,ishot,shotX,shotY,shotZ,recX,recY,recZ )
...
dist=1000*ones(1,trace_num_pershot(ishot));

    for irec=1:trace_num_pershot(ishot)
        dist(irec)=sqrt((shotX-recX(irec)).^2+(shotY-recY(irec)).^2+(shotZ-recZ(irec)).^2);
    end
